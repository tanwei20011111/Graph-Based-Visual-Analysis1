...
def load():
## Load the data
    data = pd.read_csv('./data/train.csv')[0:10005]
    ...
    # 您可以将归一化后的数据转换回 DataFrame 格式，如果需要的话
    ...
    # print(data)
    return normalized_data_df,labels_true
...
def corr_matr(intervals):
    ...
    # 示例特征向量矩阵
    # features = np.random.rand(10, 62)  # 假设这是你的特征向量矩阵
    #
    # # 计算余弦相似度矩阵
...
import math
def create_graphs(intervals,corr_matrix,labels_true,intervals_trues):
    data_list=[]
    window_size = 10
    ...
    torch.save(data_list,'./data/train0626_lstm_normal.pth')
    print(cont)
    # train1.pth w=5 train1.pth w=10

def load_data():
    dataset = torch.load('./data/train0606_lstm.pth')
    print(dataset)


if __name__ == '__main__':
    data,labels_true=load()
    intervals,intervals_trues= divide_data(data)
    corr_matrix=corr_matr(intervals)
    # print(corr_matrix)
    create_graphs(intervals,corr_matrix,labels_true,intervals_trues)
    # load_data()
